...
`src/types.ts`, `src/db/`, `src/budget.ts`, `src/providers/`, `src/prompts/`, `src/structural-check.ts`, `src/test-gate.ts`, `src/loop.ts`, `src/git.ts`, `src/pr.ts`, `src/notify.ts`, and `src/deliver.ts` exist so far — see `CLAUDE.md` for current build status.
